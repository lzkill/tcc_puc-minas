...
export class SetorComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.setorService
      ...
      .subscribe(
        (res: HttpResponse<ISetor[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: ISetor): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInSetors(): void {
    this.eventSubscriber = this.eventManager.subscribe('setorListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ISetor[] | null, headers: HttpHeaders, page: number): void {
