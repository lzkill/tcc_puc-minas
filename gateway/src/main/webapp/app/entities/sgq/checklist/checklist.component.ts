...
export class ChecklistComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.checklistService
      ...
      .subscribe(
        (res: HttpResponse<IChecklist[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IChecklist): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInChecklists(): void {
    this.eventSubscriber = this.eventManager.subscribe('checklistListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IChecklist[] | null, headers: HttpHeaders, page: number): void {
